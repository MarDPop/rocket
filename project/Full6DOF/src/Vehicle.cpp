...
void Vehicle::set_orientation(double* q) {
    ...
    q[2] *= mag;
    q[3] *= mag;
    Eigen::Quaterniond rotq(q);
    this->inertial_CS = rotq.toRotationMatrix();
}
...
void Vehicle::get_state_rate(std::array<double,14>& x, double t, std::array<double,14>& dx) {
    ...
    Eigen::Vector3d Force_in_inertial = this->inertial_CS.transpose()*this->force;

    double x_inv = 1.0/x[13];
    for(int i = 0; i < 3; i++) {
        dx[i] = x[i+3];
        dx[i+3] = Force_in_inertial(i)*x_inv + this->body.gravity->acceleration[i];
    }
...
void Vehicle::compute_symmetric_moment(double* w, double* dw) {
    dw[0] = this->moment(0)/this->inertia[0];
    dw[1] = this->moment(1)/this->inertia[1];
    dw[2] = this->moment(2)/this->inertia[2];
}

void Vehicle::compute_plane_moment(double* w, double* dw) {
    dw[1] = (this->moment(1) + this->inertia[4]*(w[2]*w[2] - w[0]*w[0]) + w[0]*w[2]*(this->inertia[0] - this->inertia[2]))/this->inertia[1];
    double y1 = this->moment(0) + w[1]*(w[2]*(this->inertia[1] - this->inertia[2]) + w[0]*this->inertia[4]);
    double y2 = this->moment(2) + w[1]*(w[0]*(this->inertia[0] - this->inertia[1]) - w[2]*this->inertia[4]);
...
void Vehicle::compute_full_moment(double* w, double* dw){
    ...
    Iw[0] = this->moment(0) - wxIw[0];
    Iw[1] = this->moment(1) - wxIw[1];
    Iw[2] = this->moment(2) - wxIw[2];
