...
void Stage::compute() {
    this->vehicle->force.setZero();
    this->vehicle->moment.setZero();

    for(auto& action : this->actions) {
        action->update(this->vehicle->TALO);
        Eigen::Vector3d torque = action->center.cross(action->force);
        this->vehicle->force += action->force;
        this->vehicle->moment += action->moment + torque;
    }
}
