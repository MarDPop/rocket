...
public:
    void step(){
		std::array<double,N> state_rate = this->dynamics->get_state_rate(this->state,this->time);
        for(int i = 0; i < N;i++){
...
public:
    ...
    void step(){
		...
		this->k0 = this->dynamics->get_state_rate(this->state,this->time);
		
		while(++i < N){
			this->state[i] = x0[i] + k0[i]*this->dt3;
		}
		
		this->k1 = this->dynamics->get_state_rate(this->state,this->time + this->dt3);
		while(--i >= 0){
			this->state[i] = x0[i] + (this->k0[i] + this->k1[i])*this->dt6;
		}

		this->k2 = this->dynamics->get_state_rate(this->state,this->time + this->dt3);
		while(++i < N){
			this->k2[i] *= 3;
			this->state[i] = x0[i] + (this->k0[i] + this->k2[i])*this->dt8;
		}

		this->k3 = this->dynamics->get_state_rate(this->state,this->time + this->dt2);
		while(--i >= 0){
			this->k3[i] *= 4;
			this->state[i] = x0[i] + (this->k0[i] - this->k2[i] + this->k3[i])*this->dt2;
		}

		this->k4 = this->dynamics->get_state_rate(this->state,this->time + this->dt);
		while(++i < N){
...
public:
    ...
    void step() {
		...
		this->x0 = this->state;
        this->k1 = this->dynamics->get_state_rate(this->state,this->time);
        ...
        while(true) {
            ...
            k2 = this->dynamics->calcStateRate(this->state,this->time + h);
            h = this->dt*0.75;
            for(;i != 0;i--){
                this->state[i] = this->x0[i] + k2[i]*h;
            }
            k3 = this->dynamics->calcStateRate(this->state,this->time + h);
            for(; i < N;i++){
            ...
            k4 = this->dynamics->calcStateRate(this->state,this->time + this->dt);
